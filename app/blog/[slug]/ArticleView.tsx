...
/**
 * Interactive part of an article page.
 *
 * `initialPost` is fetched on the server by page.tsx, which also owns the
 * metadata and JSON-LD. Seeding state from it means the article body is in the
 * server-rendered HTML instead of appearing only after a client fetch, and the
 * post is not fetched twice.
 */
export default function ArticleView({ initialPost }: { initialPost?: BlogPost | null }) {
  const { slug } = useParams();
  const [post, setPost] = useState<BlogPost | null>(initialPost ?? null);
  const [categoryName, setCategoryName] = useState<string>(initialPost?.category || '');
  const [categoryStyle, setCategoryStyle] = useState(
    (initialPost && categoryColors[initialPost.category]) || categoryColors['software-engineering']
  );
  ...
  useEffect(() => {
    async function loadData() {
      try {
        // The post itself already came from the server when initialPost is set,
        // so only the category label still needs fetching.
        const categoriesRes = await fetch('/api/categories');
        const categoriesData = await categoriesRes.json();

        let foundPost = initialPost ?? null;
        if (!foundPost) {
          const postsRes = await fetch('/api/posts');
          const postsData = await postsRes.json();
          foundPost = postsData.find((p: BlogPost) => p.slug === slug) || null;
          setPost(foundPost);
        }
