// /app/books/[slug]/chapters/[chapterNum]/ChapterView.tsx
'use client';

import Link from 'next/link';
import { useParams } from 'next/navigation';
import { useEffect, useState } from 'react';
// Shared types, so this view cannot drift from what the server passes down.
import type { Book, Chapter } from '@/app/lib/booksStore';

/**
 * Interactive part of a chapter page. The server component supplies the book,
 * the chapter, and its neighbours, so the chapter text is server-rendered and
 * the page does not refetch what it already has.
 */
export default function ChapterView({
  ...
  initialPrev?: Chapter | null;
  initialNext?: Chapter | null;
}) {
  ...
  const [prevChapter, setPrevChapter] = useState<Chapter | null>(initialPrev ?? null);
  const [nextChapter, setNextChapter] = useState<Chapter | null>(initialNext ?? null);
  const [loading, setLoading] = useState(!initialChapter);

  useEffect(() => {
    // Already supplied by the server component.
    if (initialChapter) {
      setLoading(false);
      return;
    }
  ...
  }, [slug, chapterNum, initialChapter]);
